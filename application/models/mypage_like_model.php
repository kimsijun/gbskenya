...
class mypage_like_model extends CI_Model {
    protected $table = array("mypage_like_log");
    ...
    private function _query($params) {
        ...
        if(isset($params["mpLType"])) {
            $where["mpLType"] = $params["mpLType"];
        }

        if(isset($params["mpLKey"])) {
            $where["mpLKey"] = $params["mpLKey"];
        }
    ...
    public function _like_count($params){
        $this->db->select("count(mpLKey) as cntNO , mpLType, mpLKey");
        $this->db->from($this->table[0]);
        $where = $this->_query($params);
        $this->db->where($where);
        $this->db->group_by("mpLKey");
