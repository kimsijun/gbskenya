...
class error_model extends CI_Model {
    protected $table = array("error_log");

    private function _query($params) {

        if(isset($params["erNO"])) {
            $where["erNO"] = $params["erNO"];
        }

        if(isset($params["erType"])) {
            $where["erType"] = $params["erType"];
        }

        if(isset($params["erEmail"])) {
            $where["erEmail"] = $params["erEmail"];
        }

        if(isset($params["erIsCheck"])) {
            $where["erIsCheck"] = $params["erIsCheck"];
        }
