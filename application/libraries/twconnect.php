...
class Twconnect extends TwitterOAuth {
	...
	public function Twconnect() {
		...
		$ci =& get_instance();
        if(base_url() == "http://gntv2.com/")            { $ci->config->load("twitter_local",TRUE); $config = $ci->config->item('twitter_local');
        }else if(base_url() == "http://pr.gnn.or.kr/")  { $ci->config->load("twitter_test",TRUE);  $config = $ci->config->item('twitter_test');
        }else if(base_url() == "http://goodnewstv.kr/") { $ci->config->load("twitter",TRUE);  $config = $ci->config->item('twitter');    }
