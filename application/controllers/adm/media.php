...
class media extends common {
    ...
    public function ajax_process() {
        ...
        if($params['mode'] == "getApiContent"){
            ...
            $media = strtolower($params['nwType']);
            $url =  "./assets/json/".$media."/".$params['nwCate'].".json";
            $contents = file_get_contents($url);
            $cate = json_decode($contents);

            $mediaID = ($media == "youtube")? "playlistId" : "channelId";
            $html = '<select name="nwSource" class="form-control">';
            ...
            if($params['mode'] == "videoCateUpdate") {
                ...
                $videos = $vimeo->call('vimeo.videos.getInfo', array('video_id'=>$params['nwSource']));
            ...
            if($params['mode'] == "videoCateUpdate") {
                ...
                $url = 'https://gdata.youtube.com/feeds/api/users/GoodNewsInternet/playlists?v=2&alt=json&max-results=50';
                $contents = file_get_contents($url);
                ...
                $url = 'https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails&id='.$params['nwSource'].'&fields=items&key=AIzaSyBeMZu_8m8NXoVqlU1r5DSOIhKSV04DKHo';
                //$contents = file_get_contents($url);
                $ch = curl_init();
