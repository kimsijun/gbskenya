...
class member extends common {
    ...
    public function index() {
        ...
        unset($secParams['mbLastLoginDate']);
        unset($secParams['mbVisitCount']);
        if($secParams['secTxt']){
            //$secParams['secTxt'] = iconv("euc-kr", "utf-8", urldecode($secParams['secTxt']));
            $secParams['secTxt'] = urldecode($secParams['secTxt']);

            if(!$secParams['secKey']){
                $secParams['secKey'] = "all";
                $secParams['secColumn'] = array("mbFirstName", "mbLastName", "mbID", "mbNick", "mbCellPhone");
            }
        }
        ...
        $data["total"] = $this->common_model->_select_cnt('member_data');
        $result["cnt"] = $this->member_model->_select_cnt($secParams);
        $secParams["limit"] = 10;
        $secParams["offset"] = $secParams["page"]? $secParams["page"]:0;
        $result["list"] = $this->member_model->_select_list($secParams);
    ...
    public function error_index() {
        ...
        $secParams["oKey"] = "erRegDate";
        $secParams["oType"] = "DESC";
        $data["total"] = $this->common_model->_select_cnt('login_error_log');
        $result["cnt"] = $this->common_model->_select_cnt('login_error_log',$secParams);
        $secParams["limit"] = 10;
        $secParams["offset"] = $secParams["page"]? $secParams["page"]:0;
        $result["list"] = $this->common_model->_select_list('login_error_log',$secParams);
    ...
    public function error_view() {
        $params = $this->_get_sec();
        $data = $this->common_model->_select_row('login_error_log', array('erNO'=>$params['erNO']));
        $this->_print($data);
    }

    public function error_modify() {
        $params = $this->input->post();
        $data = $this->common_model->_select_row('login_error_log', array('erNO'=>$params['erNO']));
        $this->_print($data);
    }
    ...
    public function process () {
        ...
        } else if($params['mode'] == "error_modify"){
            unset($params['mode']);
            $params['erModDate'] = 'NOW()';
            $this->common_model->_update('login_error_log', $params,array('erNO'=>$params['erNO']));
            $path = "./_cache/%cache"; delete_files($path, true);
            redirect(base_url("/adm/member/error_index"));
        }
    }
